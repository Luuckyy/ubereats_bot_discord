...
const ping:Command = {
	data: async function() {return new SlashCommandBuilder()
		.setName('setrole')
		.setDescription('Set role that will be mentionned at every message (make sure role is mentionnable)')
        .addMentionableOption(option => option.setName("role").setDescription("THE role").setRequired(true))
	},
